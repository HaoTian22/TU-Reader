 ...
 * MAX(row_id) 变更）才重建并回写。属于"缓存"而非用户数据：设置页「清理缓存」会连同 transit.db
 * 一起清掉，系统也允许在存储紧张时回收 cacheDir（缓存缺失时只是慢一点重新构建）。
 *
 * 缓存键 = 该卡 transactions_archive 的 MAX(row_id)：新交易落库即单调增长 → 缓存失效。
 * 站名映射表在线更新 / 显示语言切换不靠此键，由调用方显式清缓存或强制重建。
 */
...
object UiCache {
    ...
    /** 命中需同时满足：文件存在、格式版本一致、archive row_id 未变化；损坏/不匹配返回 null（当作无缓存） */
    fun load(context: Context, cardId: String, archiveRowId: Long): CardUiCache? {
        val f = file(context, cardId)
        if (!f.exists()) return null
        return try {
            val c = gson.fromJson(f.readText(), CardUiCache::class.java)
            if (c.version != CardUiCache.FORMAT_VERSION || c.archiveRowId != archiveRowId) null else c
        } catch (e: Exception) {
            null
        }
    }
